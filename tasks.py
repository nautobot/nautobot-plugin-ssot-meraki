...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_NAUTOBOT_SSOT_MERAKI_xxx
namespace = Collection("nautobot_ssot_meraki")
namespace.configure(
    {
        "nautobot_ssot_meraki": {
            "nautobot_ver": "2.3.1",
            "project_name": "nautobot-ssot-meraki",
            "python_ver": "3.11",
            "local": False,
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.nautobot_ssot_meraki.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.nautobot_ssot_meraki.compose_http_timeout,
        "NAUTOBOT_VER": context.nautobot_ssot_meraki.nautobot_ver,
        "PYTHON_VER": context.nautobot_ssot_meraki.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.nautobot_ssot_meraki.project_name}",
        f'--project-directory "{context.nautobot_ssot_meraki.compose_dir}"',
    ]

    for compose_file in context.nautobot_ssot_meraki.compose_files:
        compose_file_path = os.path.join(context.nautobot_ssot_meraki.compose_dir, compose_file)
...
def run_command(context, command, **kwargs):
    ...
    if is_truthy(context.nautobot_ssot_meraki.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.nautobot_ssot_meraki.python_ver}...")
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.nautobot_ssot_meraki.python_ver
    dockerfile_path = os.path.join(context.nautobot_ssot_meraki.compose_dir, "Dockerfile")
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=False):
    ...
    if constrain_nautobot_ver:
        ...
        if constrain_python_ver:
            command += f" --python {context.nautobot_ssot_meraki.python_ver}"
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.nautobot_ssot_meraki.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {context.nautobot_ssot_meraki.python_ver}"
            run_command(context, command)
    else:
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations nautobot_ssot_meraki"
...
@task(
    help={
        "version": "Version of Nautobot SSoT Meraki to generate the release notes for.",
    }
)
...
def pylint(context):
    ...
    command = 'pylint --init-hook "import nautobot; nautobot.setup()" --rcfile pyproject.toml nautobot_ssot_meraki'
    run_command(context, command)
...
def unittest_coverage(context):
    ...
    command = "coverage report --skip-covered --include 'nautobot_ssot_meraki/*' --omit *migrations*"
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.nautobot_ssot_meraki.local):
