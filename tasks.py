...
namespace.configure(
    {
        "nautobot_ssot_meraki": {
            "nautobot_ver": "latest",
            "project_name": "nautobot_ssot_meraki",
            "python_ver": "3.8",
            "local": False,
...
def dbshell(context, query="", input="", output=""):
    ...
    if "docker-compose.mysql.yml" in context.nautobot_ssot_meraki.compose_files:
    ...
    elif "docker-compose.postgres.yml" in context.nautobot_ssot_meraki.compose_files:
        command += [
            "--",
            service,
            "psql",
...
def unittest(context, keepdb=False, label="nautobot_ssot_meraki", failfast=False, buffer=True, pattern=""):
